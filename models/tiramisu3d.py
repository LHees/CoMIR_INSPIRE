...
class TransitionDown3D(nn.Sequential):
    def __init__(
        ...
        if dropout and dropout != 0.0:
            self.add_module("dropout", nn.Dropout3d(0.2))
        if pooling == "max":
            self.add_module("pool", nn.MaxPool3d(2))
        elif pooling == "avg":
            self.add_module("pool", nn.AvgPool3d(2))
...
def center_crop3D(layer, max_depth, max_height, max_width):
    ...
    z_start = (d - max_depth) // 2
    return layer[:, :, y_start:y_start + max_height,
                 x_start:x_start + max_width, z_start:z_start + max_depth]


class DenseUNet3D(BaseModel):
    """DensUNet in 3D
    ...
    def __init__(
            ...
            init_conv_stride=1,
            down_blocks=(4, 4, 4),  # was (4)*5
            bottleneck_layers=4,
            up_blocks=(4, 4, 4,),  # was (4)*5
            growth_rate=12,
            compression=1.0,
            dropout_rate=0.2,
